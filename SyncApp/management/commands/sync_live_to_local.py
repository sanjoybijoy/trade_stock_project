...
class Command(BaseCommand):
    help = "Sync live database with local database"

    def handle(self, *args, **options):
        live_dump_file = 'live_dump.sql'
        ...
        try:
            ...
            dump_cmd = (
                ...
                f"--ignore-table=sanjoybijoy$stockiq_01.django_session > {live_dump_file}"
            )
            subprocess.call(dump_cmd, shell=True)

            self.stdout.write("Importing into local database...")
            import_cmd = (
                f"mysql -u root -pm4001chandra stockiq_01 < {live_dump_file}"
            )
            ...
            if os.path.exists(live_dump_file):
                os.remove(live_dump_file)
