...
def fetch_and_save_earnings(symbol, event_name=None, earning_call_time=None):
    """
    ...
    Optional event_name and earning_call_time can be manually set for Slot 1.
    """
    ...
    for i in range(1, 5):
        defaults.update({
            ...
            f"event_name_{i}": 'Others',
            f"earning_call_time_{i}": 'Others',
        })
    ...
    # Apply manual overrides for Slot 1 if provided
    if event_name:
        defaults["event_name_1"] = event_name
    if earning_call_time:
        defaults["earning_call_time_1"] = earning_call_time
...
def update_multiple_tickers_earnings(tickers_string, event_name=None, earning_call_time=None):
    tickers = [t.strip().upper() for t in tickers_string.split(',') if t.strip()]
    summary = []
    for symbol in tickers:
        try:
            msg = fetch_and_save_earnings(symbol, event_name=event_name, earning_call_time=earning_call_time)
            summary.append(msg)
        except Exception as e:
